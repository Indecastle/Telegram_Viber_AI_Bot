...
public class TelegramOpenAiGenerationImageTool(ITelegramBotClient botClient, OpenAiConfiguration openAiOptions)
{
    public async Task<string> UploadImageToImgure(MemoryStream stream, CancellationToken cancellationToken)
    {
        ...
        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Client-ID", openAiOptions.ImgurToken);
    ...
    public async Task<string> GenerateAndSendPhotoAsync(Message message, string prompt, CancellationToken cancellationToken)
    {
        var photolink = await GenerateFusionbrainImage(prompt, cancellationToken);
        await botClient.SendPhotoAsync(message.Chat.Id, new InputFileUrl(photolink), cancellationToken: cancellationToken);
        return photolink;
    }
    ...
    private async Task<string> GenerateFusionbrainImage(string prompt, CancellationToken cancellationToken)
    {
        ...
        var response = await httpClient.PostAsync("https://api-key.fusionbrain.ai/key/api/v1/text2image/run", requestContent);
    ...
    private async Task<string> GetGeneratedImageAsync(HttpClient httpClient, string uuid)
    {
        for (int i = 0; i < 20; i++)
        {
            var resultStatus = await httpClient.GetAsync("https://api-key.fusionbrain.ai/key/api/v1/text2image/status/" + uuid);
            var resultStatusJson = JsonSerializer.Deserialize<JsonObject>(await resultStatus.Content.ReadAsStreamAsync());
